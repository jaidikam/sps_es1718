...
#count.fields("GNI.csv")

#NAMES <- read.table("GNI.csv", nrow = 1, stringsAsFactors = FALSE, sep = ",")
#DATA <- read.table("GNI.csv", skip = 1, stringsAsFactors = FALSE, sep = ",")
#DATA <- DATA[, 1:24]
#names(DATA) <- NAMES 
...
colnames(importData) <- c("Year", "Value")
...
colnames(exportData) <- c("Year", "Value")
...
saveRDS(data, "Philippines.rds")

inda = readRDS("/Users/Stefan/sps_ws1718")
